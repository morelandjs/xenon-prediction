...
def plot(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        for fname in ('{}.png', '{}.pdf'):
            plotfile = plotdir / fname.format(f.__name__)
    ...
    for plot, ax, ratio_ax in zip(plots, axes[::2].flat, axes[1::2].flat):
        for system, (obs, subobs, opts) in itertools.product(
            ...
            handles['model'][system] = \
                lines.Line2D([], [], color='k', ls=linestyle)
            ...
            handles['expt'][system] = ax.errorbar(
                x, yexp, yerr=yerrstat, fmt='o', capsize=0, color='k',
                mfc=('k' if fill_markers else '.9'),
                mec='k', mew=(0 if fill_markers else .25),
                zorder=1000
            )
...
def nch_per_npart():
    ...
    plt.errorbar(
        ...
        color='k', fmt='o', label='ALICE: Pb-Pb, 5.02 TeV'
    )
